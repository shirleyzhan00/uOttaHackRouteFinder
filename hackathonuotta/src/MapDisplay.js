...
const MapDisplay = ({ sourceAddress, destAddress }) => {
            ...
            directionsService.route(request, (result, status) => {
                if (status === "OK") {
                    ...
                    while (currentStepIndex < points.length - 1) {
                        ...
                        const stepDistance = window.google.maps.geometry.spherical.computeDistanceBetween(currentPoint, nextPoint);
                        ...
                        if (currentDistance >= nextStepDistance) {
                            ...
                            const newPoint = window.google.maps.geometry.spherical.computeOffset(currentPoint, remainingDistance, heading);
    ...
    return (
        <>
            ...
            {clickedMarker && (
                <EvStation lat={clickedMarker.lat()} lng={clickedMarker.lng()} />
            )}
        </>
    );
};
